...
export default class extends Controller {
  ...
  .then(html => {
    ...
    const newArticles = doc.querySelectorAll('#articles .article'); // Adjust selector to match article elements
    
    newArticles.forEach(article => {
      if (!document.querySelector(`#articles .article[data-id="${article.dataset.id}"]`)) { // Prevent duplicates
        this.articlesTarget.insertAdjacentHTML('beforeend', article.outerHTML);
      }
    });
